...
| --- | ---: | ---: |
| Proposal Detail screen-owner lines | 827 | 665 |
...
The extracted 243-line query-state owner contains the four source queries, serial persisted-command
mutations, exact invalidation/refetch transaction, current-version confirmation, and historical
version lookup. It also retains persisted-but-unconfirmed command failures at proposal scope so a
route remount cannot silently restore an action. It does not calculate or infer proposal, approval,
workflow, or lineage truth.
...
- `tests/integration/proposal-detail-view.test.tsx` proves source-confirmed lifecycle and version
  ...
  retained confirmed version evidence on revisit, and persisted-confirmation fencing after leaving
  and returning to the same proposal.
