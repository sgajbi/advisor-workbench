...
| DPM PM operating quality | `/workbench/{portfolioId}?mode=quality` | Gateway `/api/v1/dpm/command-center/pm-operating-quality*` | Implemented for Manage-owned policy, score-run, source-defined segment, fairness-analysis preview/create/list/detail, score-run support-summary request, and bounded supervisory review-action preview/create/list/detail through Gateway only. Review-action create is preview-gated and records Manage-owned supervisory evidence; Workbench does not calculate PM scores, fairness spreads, segment membership, PM rankings, HR/conduct posture, client communication, trade/order, OMS, execution, fills, or settlement truth. |
