...
| Portfolio bundle intake                 | `/intake`                                                           | Gateway `/api/v1/intake/portfolio-bundle` through the Workbench BFF                                                                                                                                                                                                                                                                     | Supported for list-built and CSV portfolio bundle submission through Gateway only. Workbench forwards a bounded `X-Idempotency-Key`, reuses it for same-payload retry after a failed attempt, and clears it after success while leaving ingestion replay, duplicate handling, lineage, and durable job truth to Gateway/Core.                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| DPM mandate command center              | `/workbench/{portfolioId}`, `/workbench/{portfolioId}?mode=mandate` | Gateway `/api/v1/dpm/command-center*`                                                                                                                                                                                                                                                                                                   | Supported for embedded canonical mandate cockpit, PM-book-backed monitoring action, active exception queue, and governed exception-summary request through Gateway/Manage/lotus-ai. Returned exception summaries use the shared business disclosure for output availability, evidence, review, client use, and freshness. Workbench preserves Manage supportability posture: populated canonical `READY` is demo-ready, `PARTIAL`/`DEGRADED`/`BLOCKED` render as explicit partial states, and `EMPTY` stays an empty state rather than a false ready cockpit.                                                                                                                                                                                                                                                           |
