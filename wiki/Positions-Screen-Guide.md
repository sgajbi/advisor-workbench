...
Positions is the selected portfolio's booked inventory and position-review workspace. It brings
...
| Primary next action | Review positions needing attention, then open the owning Transactions or Portfolio Review workflow |

The screen uses the shared portfolio context. Changing the portfolio re-requests the selected
portfolio's source records; it does not combine positions across clients, households, mandates, or
...
2. Which positions contribute the portfolio value, cost, weight, and unrealised result?
3. Which valuation or source-status evidence is incomplete or requires review?
4. What recent booked activity explains a selected position?
...
- **Client advisors and relationship managers** use it to prepare portfolio reviews and answer
  position-level client questions from the selected mandate's booked evidence.
- **Portfolio managers and investment specialists** use it to inspect exposure, valuation, cost,
  position status, and recent activity before opening the relevant analytical or operating workflow.
...
5. Select a position to review overview, valuation, and recent booked activity without leaving the
   inventory.
...
- Uses one fail-closed position-state projection across the grid, CSV export, and evidence rail:
  ...
  **Review required**; an absent state becomes **Not reported**; and a synthesized cash-balance row
  is **Not applicable**.
- Shows incomplete price or valuation evidence without dropping the positions that remain usable.
- Opens a position drawer with overview, valuation, and recent activity tabs plus a supported handoff
...
| --- | --- | --- |
| Search positions | A loaded booked inventory | None; this filters the current grid only |
| Choose or expand columns | A loaded grid | None; this changes presentation only |
| Review a position | Select a visible booked security or cash balance | None; the detail drawer opens over current source evidence |
| Export positions | Current rows and visible-column policy | A local CSV download only |
| Open Transactions | A selected portfolio; a selected position is optional | None on Positions; the user enters the booked-activity workflow |
...
| Recent position activity | Filtered from the source-returned transaction window by security or instrument id | Core booked transaction records composed by Gateway |
...
| Missing valuation | The affected positions remain visible with a partial-valuation message and blank unavailable figures | Review the named positions; do not infer zero price, value, or P&L |
| Position status not reported | The row and export say **Not reported** and the evidence rail becomes **Partial** | Confirm the source position status before relying on the position's currency |
...
| Activity unavailable | Position overview and valuation remain available while recent activity is explicitly unavailable | Open Transactions when ledger detail is restored |
...
- [Portfolio Review](Portfolio-Review-Screen-Guide) provides the selected mandate's daily decision
  checkpoint.
- Allocation explains direct exposure contribution from the booked positions.
...
- Portfolio component and production-browser validation cover the shared grid, position drawer,
