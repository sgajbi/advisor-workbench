...
| Source advisory date invalid | **Portfolio evidence date is unavailable**, the rejected source date, retained non-authoritative facts, and unavailable proposal actions | Refresh portfolio evidence so the source can provide a valid date; Workbench locks repeated refresh while retrieving the replacement and does not normalize or substitute the returned value |
