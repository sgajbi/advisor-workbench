...
- Restores an addressed transaction beyond the loaded ledger page through Gateway's exact-record
  ...
  non-projected posture. One hydration performs one read: window focus, reconnect, and successful or failed component remount reuse its
  session result even after the normal inactive-query collection window, while **Retry transaction**
  explicitly recontacts Gateway for the same context.
...
- Exact-record tests prove one direct request across window focus, reconnect and successful or failed remount beyond the normal collection window, explicit recovery,
