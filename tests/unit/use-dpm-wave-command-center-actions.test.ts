...
describe("useDpmWaveCommandCenterActions", () => {
  ...
  it("suppresses a retained created wave when the current list load is unavailable", async () => {
    const createdResponse = buildCreatedWaveResponse();
    const confirmedItems: DpmWaveGatewayResponse = {
      ...itemResponse,
      supportability: {
    ...
    expect(unavailableMount.result.current.model.selectedWaveId).toBeNull();

    act(() => unavailableMount.result.current.requestApproval());
    expect(approveDpmWave).not.toHaveBeenCalled();
  });
  ...
  it("retires command context when the source list absorbs its wave", async () => {
    const previewCandidate: DpmWaveGatewayResponse = {
      ...waveResponse,
      supportability: { ...waveResponse.supportability, wave_id: "dwv_003" },
      data: { wave: { wave_id: "dwv_003", state: "SIMULATION_READY" } },
    ...
    const absorbedList: DpmWaveGatewayResponse = {
      ...waveResponse,
      correlation_id: "corr-list-absorbed-candidate",
      supportability: { ...waveResponse.supportability, wave_id: "dwv_003" },
      data: {
        items: [
          {
    ...
    await waitFor(() =>
      expect(
        queryClient.getQueryData(
          dpmWaveQueryKeys.commandContext("PB_SG_GLOBAL_BAL_001"),
        ),
      ).toBeUndefined(),
    );
    expect(mounted.result.current.model.selectedWaveId).toBe("dwv_003");
  ...
  it("preserves the wave-dependent request fence across a remount", async () => {
    const previewCandidate: DpmWaveGatewayResponse = {
      ...waveResponse,
      supportability: { ...waveResponse.supportability, wave_id: "dwv_003" },
      data: { wave: { wave_id: "dwv_003", state: "SIMULATION_READY" } },
    ...
    const approvedCandidate: DpmWaveGatewayResponse = {
      ...previewCandidate,
      correlation_id: "corr-approved-remounted-candidate",
      supportability: {
        ...previewCandidate.supportability,
        wave_state: "APPROVED",
      },
    ...
    act(() => {
      ...
      secondMount.result.current.requestOperationsBrief();
      secondMount.result.current.loadProposedChanges();
      secondMount.result.current.openEvidencePack();
    });

    expect(requestDpmWaveAiPmMemo).not.toHaveBeenCalled();
    expect(requestDpmOperationsHandoffSummary).not.toHaveBeenCalled();
    expect(getDpmWaveItems).toHaveBeenCalledTimes(itemReadCount);
