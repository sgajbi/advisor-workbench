...
test.describe('UI smoke checks', () => {
  test('portfolio foundation page renders core sections', async ({ page }) => {
    await page.goto(`${baseUrl}/portfolios`, { waitUntil: 'domcontentloaded' });
    await expect(page.getByRole('heading', { name: /Portfolio Foundation/i })).toBeVisible();
    await expect(page.getByText(/Existing Portfolios|No portfolios are currently available/i)).toBeVisible();
  });
